...
const UpdateInvoice = (props) => {
    ...
    const totalIncomingMoney = transferMoneyDetail
        .filter(trans => trans.TicketNo === ticket.No && trans.TicketType === "ตั๋วรถใหญ่" && trans.Status !== "ยกเลิก")
        .reduce((sum, trans) => {
    ...
    const totalIncomingDetail = transferMoneyDetail
        .filter(trans => trans.TicketNo === ticket.No && trans.Status !== "ยกเลิก")
            ...
            <Grid container spacing={2}>
                <Grid item md={12}>
                    <Typography variant="subtitle1" sx={{ fontSize: "18px", marginBottom: -2 }} fontWeight="bold" gutterBottom>
                        รายละเอียดการชำระน้ำมันวันที่ส่ง : {dayjs(ticket.Date, "DD/MM/YYYY").format("D เดือนMMMM พ.ศ.BBBB")} / ตั๋ว : {ticket.TicketName.split(":")[1]}
                    </Typography>

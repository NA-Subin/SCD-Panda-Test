    ...
    const saveProduct = () => {
        ...
        const updatedVolume = gasStationOil.reduce((acc, row) => {
            Object.entries(row.Products).forEach(([key, value]) => {
                const matchingStock = stock.find((s) => s.ProductName === key);
                if (matchingStock) {
                    acc[key] = (acc[key] || 0) + Number(value) + Number(newVolumes[key] || 0);
                }
            });
            return acc;
        }, {});
                                ...
                                <Grid item xs={3.5} md={2} lg={1.5}>
                                    ...
                                    <Paper component="form" sx={{ marginTop: -1 }}>
                                        <TextField
                                            ...
                                            type="text"
                                            fullWidth
                                            value={setting ? (new Intl.NumberFormat("en-US").format(Number(row.Volume))) : (new Intl.NumberFormat("en-US").format(Number(row.OldVolume) + Number(updateVolumes[row.ProductName] || 0)))}
                                            disabled
                                        />
                                    </Paper>
                                </Grid>
